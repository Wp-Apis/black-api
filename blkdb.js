...
return res.json({message: `✔️ ${rq} foram adc a apikey ${apikey} com sucesso...`})
} else {
...
fs.writeFileSync("./database/apikeys.json", JSON.stringify(key))
return res.json({message: `✔️ ${request} requests foram adc a apikey ${apikey} com sucesso...`})
} else {
key.push({apikey: apikey, request: Number(request)})
fs.writeFileSync("./database/apikeys.json", JSON.stringify(key))
return res.json({message: `✔️ _Apikey registrada com sucesso ↴_
🔑 *Key:* ${apikey}
💾 *Requests:* ${request}`})
}
})
...
var senhaofc = senhapv
if(senha != senhaofc) return res.json({message: "[❗] senha inválida ou inexistente ❌"})
if(!apikey) return res.json({message: "Qual é a apikey que você deseja remover?"})
if(!key.map(i => i.apikey).includes(apikey)) {
return res.json({message: "[❗] apikey não encontrada ou inexistente ❌"})
} else {
var i2 = key.map(i => i.apikey).indexOf(apikey)
...
return res.json({status: true, message: "Key ativa ✅", limite_de_request: key[ITC]?.request, consulta: key[ITC]?.consulta, horarios: key[ITC]?.horarios})
}
})
...
if(!query) return res.json({resultado: `KD o parâmetro query?`})
if(!apikey) return res.json({resultado: `KD o parâmetro apikey?`})
if(!key.map(i => i.apikey)?.includes(apikey)) return res.sendFile(path.join(__dirname, "./public/", "apikey_invalida.html"))
if(key[key.map(i => i?.apikey)?.indexOf(apikey)]?.request <= 0) return res.json({message: "Apikey inválida ou requests esgotados!"})
RegisKey(apikey, req);
try {
...
if(!mod) return res.json({resultado: "Faltando o parâmetro mod"})
...
try {
nome = req.query.nome
if(!nome)return res.json({status:false, motivo:'Coloque o parâmetro: nome'})
apikey = req.query.apikey;
if(!apikey)return res.json({status:false,message:'- Cadê o parâmetro apikey?'})
if(!key.map(i => i.apikey)?.includes(apikey))return res.sendFile(path.join(__dirname, "./public/", "apikey_invalida.html"))
if(key[key.map(i => i?.apikey)?.indexOf(apikey)]?.request <= 0) return res.json({message: "Apikey inválida ou requests esgotados!"})
...
res.json({status: 200, resultado: {thumb: signo.imagem, title: signo.título, info: signo.inform}})
}).catch(e => {
...
res.json({status: true, código: 200, criador: `@m4thxyz_`, resultado: resultado})
}).catch(e => {
res.json({
message: `Erro no Servidor Interno`
})})})
...
var { url, apikey } = req.query
if(!url) return res.json({resultado: `Faltando o parâmetro url`})
if(!apikey) return res.json({resultado: `Faltando o parâmetro apikey`})
if(!key.map(i => i.apikey)?.includes(apikey)) return res.sendFile(path.join(__dirname, "./public/", "apikey_invalida.html"))
if(key[key.map(i => i?.apikey)?.indexOf(apikey)]?.request <= 0) return res.json({message: "Apikey inválida ou requests esgotados!"})
RegisKey(apikey, req);
...
if(!usuario) return res.json({status:false,message:'Cadê o parâmetro: usuario'})
if(!apikey) return res.json({status:false,message:'Cadê o parâmetro: apikey'})
if(!key.map(i => i.apikey)?.includes(apikey)) return res.sendFile(path.join(__dirname, "./public/", "apikey_invalida.html"))
if(key[key.map(i => i?.apikey)?.indexOf(apikey)]?.request <= 0) return res.json({message: "Apikey inválida ou requests esgotados!"})
RegisKey(apikey, req)
